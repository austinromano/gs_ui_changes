...
{
    *aliveFlag = false;   // prevent any queued callAsync lambdas from firing
    disconnect();
}
...
void WebSocketConnection::disconnect()
{
    ...
    // Notify listeners synchronously — callAsync would capture 'this'
    // which may already be destroyed when the lambda runs.
...
void WebSocketConnection::handleMessage(const juce::String& json)
{
    ...
    {
        juce::MessageManager::callAsync([this, weak, payload] {
            if (weak.expired()) return;
    ...
    else if (type == "transport_sync")
    {
        ...
        juce::MessageManager::callAsync([this, weak, beat, ts] {
            if (weak.expired()) return;
    ...
    else if (type == "presence")
    {
        ...
        juce::MessageManager::callAsync([this, weak, userId, online] {
            if (weak.expired()) return;
    ...
    else if (type == "invite")
    {
        ...
        juce::MessageManager::callAsync([this, weak, code, hostName] {
            if (weak.expired()) return;
