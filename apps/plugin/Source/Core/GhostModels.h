...
//==============================================================================
struct GhostSessionFile
{
    juce::String id;
    juce::String fileName;
    int          fileSize = 0;
    juce::String uploadedBy;
    juce::String uploaderName;
    juce::String createdAt;

    static GhostSessionFile fromJson(const juce::var& v)
    {
        GhostSessionFile f;
        if (auto* obj = v.getDynamicObject())
        {
            f.id           = obj->getProperty("id").toString();
            f.fileName     = obj->getProperty("fileName").toString();
            f.fileSize     = static_cast<int>(obj->getProperty("fileSize"));
            f.uploadedBy   = obj->getProperty("uploadedBy").toString();
            f.uploaderName = obj->getProperty("uploaderName").toString();
            f.createdAt    = obj->getProperty("createdAt").toString();
        }
        return f;
    }
};
